...
const docsSrc = path.resolve(docsDir, 'src');
const entryFile = path.resolve(docsSrc, './index.html');
const contentFileDir = path.resolve(docsSrc, 'content');
const rawFileDir = path.resolve(docsSrc, 'raw');
const output = path.resolve(docsDir, 'dist');
...
// Options du paquet
const options = {
  watch: false, // Surveiller les fichiers et les reconstruire lors d'un changement, par défaut pour process.env.NODE_ENV !== 'production'
  outDir: output, // Le répertoire out pour mettre les fichiers de construction, par défaut dist
